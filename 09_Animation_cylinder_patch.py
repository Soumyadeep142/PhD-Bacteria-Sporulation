...
# --- Build the full cell shape as a shapely polygon (hemisphere + cylinder) ---
def build_cell_polygon():
    ...
    # Combine: hemisphere from back -> top -> cylinder front -> bottom -> back
...
# --- Static: Draw cell (hemisphere + cylinder separately for color) ---

light_green_c='#90ee90'
deep_green_c='#006400'


# Hemisphere (deep green)
theta = np.linspace(np.pi / 2, 3 * np.pi / 2, 100)
hemi_x = cell_radius * np.cos(theta)
hemi_y = cell_radius * np.sin(theta)
hemi_patch = patches.Polygon(np.column_stack((hemi_x, hemi_y)),
                              closed=True, facecolor=light_green_c, ec='black', lw=2, alpha=0.6)
ax.add_patch(hemi_patch)

# Cylinder (light green)
cyl_patch = patches.Rectangle((0, -cell_radius), cylinder_length, 2 * cell_radius,
                              facecolor=light_green_c, ec='black', lw=2, alpha=0.6)
ax.add_patch(cyl_patch)

# --- Patch for the intersection (animated) ---
intersection_patch = PathPatch(Path.make_compound_path(), facecolor=deep_green_c, ec='black', alpha=0.6)
ax.add_patch(intersection_patch)

# --- Helper: convert shapely polygon to matplotlib path patch ---
...
def animate(i):
    ...
    # Convert to patch and update
    new_patch = polygon_to_pathpatch(intersection, facecolor='blue', alpha=0.6, edgecolor='none')
...
# --- Save as MP4 ---
ani.save("spore_cell_intersection_with_geometry.mp4", writer='ffmpeg', fps=300)
